...
type UniversalHeroProps = {
  ...
  visualTone?:
    ...
    | "personal"
    | "explorer";
  minHeight?: "compact" | "standard" | "immersive";
  align?: "left" | "center";
  mediaSlot?: ReactNode;
  metadata?: ReactNode;
  actions?: ReactNode;
...
export default function UniversalHero({
  ...
  return (
    <section
      className={`relative overflow-hidden rounded-[var(--atlas-radius-hero)] border border-[var(--atlas-border)] bg-[var(--atlas-bg-elevated)] p-8 shadow-[0_28px_90px_rgba(0,0,0,0.34)] md:p-12 ${getHeightClass(
        minHeight
      )}`}
    >
      {backgroundImage && (
      ...
      <div
        className={`relative z-10 flex min-h-48 max-w-3xl flex-col justify-center ${
      ...
      >
        ...
        <h1 className="mt-4 text-5xl font-semibold leading-tight text-[var(--atlas-text-on-image)] md:text-6xl">
          {title}
        </h1>
...
function getOverlayClass(overlayStrength: UniversalHeroProps["overlayStrength"]) {
  const classes = {
    ...
    strong:
      "bg-[linear-gradient(110deg,rgba(0,0,0,0.9)_0%,rgba(0,0,0,0.72)_42%,rgba(0,0,0,0.28)_100%)]",
  };
